/**
 * raspi_gpio_driver.c - GPIO Linux device driver for Raspberry Pi Zero
 * rev 2.0 platform
 * License: GPL
 */
 ...
 *
 * This feature is pretty experient, so more work needs to
 ...
 * set to input, then allow the spedified GPIO pin to set interrupt.
 */
 ...
 *
 * This functions release GPIO interrupt resource when the device is
 ...
 * "diable-irq" Disable interrupt on a GPIO pin
 */
                  ...
                  loff_t *f_pos)
{
    ...
    if (strcmp(kbuf, "out") == 0) {
        ...
        sscanf(kbuf, "%d", &value);
        if (raspi_gpio_devp->dir == in) {
            printk(KERN_ERR "Cannot set GPIO %d, direction : input\n", gpio);
            return -EPERM;
        }
        if (raspi_gpio_devp->dir == out) {
            if (value > 0) {
...
/**
 * Create mydevnode function
 */
static char *mydevnode(struct device *dev, umode_t *mode) {
    if (mode) {
        *mode = 0666; /* set the permissions to all groups*/
        /* could override /dev name here too */
    }
    return NULL;
}

/**
 * raspi_gpio_init - Initiallize GPIO device driver
 *
...
raspi_gpio_init(void)
{
    ...
    if ((raspi_gpio_class = class_create( THIS_MODULE,
                                          DEVICE_NAME)) == NULL) {
        printk(KERN_DEBUG "Cannnot create class %s\n", DEVICE_NAME);
        unregister_chrdev_region(first, NUM_GPIO_PINS);
        return -EINVAL;
    }
    raspi_gpio_class->devnode = mydevnode;
